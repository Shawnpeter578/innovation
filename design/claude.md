...
seo:
  ...
  metaDescription: "Anthropic's Claude design system as a DESIGN.md file. Cream canvas #faf9f5, coral #cc785c, Copernicus serif, 30 components. For React, Next.js, and AI tools."
  highlights:
    - "Cream canvas #faf9f5 with warm ink #141413 — the defining counter-positioning against every cool-gray AI brand"
    - "Coral primary #cc785c used scarcely on CTAs and generously on full-bleed coral callout cards, never as a third accent"
    - "Copernicus slab-serif display at weight 400 with -1.5px tracking — the literary voice that separates Claude from sans-led SaaS"
    - "Three surface modes alternate page-by-page — cream canvas, cream cards #efe9de, and dark navy product mockups #181715"
    - "Generous 32px card padding and 96px section rhythm — pacing reads as long-form magazine column, not marketing template"
  tags:
    - "AI & LLM Platforms"
  ...
  questions:
    - id: "primary-color"
      title: "What is Claude's primary brand color?"
      answer: "Claude's signature color is a warm coral at #cc785c, with an active press state at #a9583e and a disabled cream-tinted #e6dfd8. The coral carries every primary CTA, full-bleed callout cards, and inline links in body copy. It is deliberately warm and slightly muted — never cyan or saturated red — to counter-position against OpenAI's slate, Google's saturated blue, and Microsoft's corporate cyan. The system uses it scarcely on individual buttons and generously on entire coral surface bands."
    - id: "typography"
      title: "What typography does Claude use, and what should I use if Copernicus isn't available?"
      answer: "Claude runs Copernicus (or Tiempos Headline) as a slab-serif display at weight 400 for h1 through h3, paired with StyreneB or Inter as the humanist sans body. JetBrains Mono handles code. Display sizes ladder from 28px to 64px with negative letter-spacing from -0.3px to -1.5px — the tracking is non-negotiable. If Copernicus is unavailable, Cormorant Garamond at weight 500 with -0.02em tracking is the closest open-source substitute. For StyreneB, Inter or Söhne work cleanly."
    - id: "surface-modes"
      title: "Why does Claude alternate between cream and dark navy surfaces?"
      answer: "The system uses three surface modes that rotate page-by-page: cream canvas #faf9f5 as the default floor, cream feature cards #efe9de one shade darker, and dark navy product surfaces #181715 for code editor mockups, model showcase cards, and pre-footer CTAs. The cream-to-dark contrast is the page's pacing rhythm. Dark surfaces are where Claude shows its actual product chrome — code blocks, terminal output, model comparison tables — rather than abstract marketing illustrations."
    - id: "use-in-project"
      title: "Can I use this DESIGN.md to build my own React app?"
      answer: "Yes — the file is structured to feed into Claude, Cursor, or any AI tool that reads design tokens. The agent reproduces Claude's editorial voice (serif headlines, cream canvas, coral CTAs, dark code-window cards) instead of a generic shadcn theme. Every color, type style, radius, and spacing value is a quoted token you can paste into Tailwind config, CSS variables, or your own component library. Border radii ladder from 4px buttons through 8px CTAs, 12px cards, and 16px hero illustrations."
    - id: "known-gaps"
      title: "What's missing from this DESIGN.md spec?"
      answer: "A handful of items documented in the Known Gaps section. Copernicus and StyreneB are licensed Anthropic typefaces with no public web fonts — substitutes are listed but not exact. The Anthropic radial-spike-mark is treated as a logo asset, not a system token. Animation and transition timings (chat message reveals, code typewriter, agentic-flow diagrams) are out of scope. Form validation states beyond focus are not extracted. The claude.ai product surface adds chat bubbles, message tools, and conversation sidebars not captured in this marketing-surface document."
